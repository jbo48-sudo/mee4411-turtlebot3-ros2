...
def test_transform2xyt():
    ...
    for v, v_true, name in zip(pose, pose_true, names):
        assert np.abs(v - v_true) < 1e-4, \
            f'{name} has the wrong value, {v:.6f} instead of {v_true:.6f}'


# test xyt2transform
def test_xyt2transform():
    # Set up test inputs
    x = -2.9
    y = 2.3
    theta = -0.4
...
def test_xyt2homogeneous():
    ...
    # Ensure that calculated outputs match desired outputs
    assert T.shape[0] == 3, f'Homogeneous matrix has {T.shape[0]} rows instead of 3'
    assert T.shape[1] == 3, f'Homogeneous matrix has {T.shape[1]} cols instead of 3'
    for r in range(2):
        for c in range(2):
            assert np.abs(T[r, c] - T_true[r, c]) < 1e-4, \
                f'Element in ({r:d},{c:d}) has the wrong value, \
{T[r, c]:.6f} instead of {T_true[r, c]:.6f}'
