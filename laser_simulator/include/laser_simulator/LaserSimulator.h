...
#include "laser_simulator/msg/pose_stamped_named.hpp"
...
typedef struct Model
{
  ...
  Model(double x, double y, double z) : xdim(x), ydim(y), zdim(z) {
  }
} model_t;
...
public:
  ...
  void LoadOccupancyGrid(const nav_msgs::msg::OccupancyGrid & map, double depth);
  void UpdatePoseArray(const laser_simulator::msg::PoseStampedNamedArray & pose_array);
  int LoadLaserModel(std::shared_ptr < rclcpp::Node > n);
  // int LoadDynamicModels(const rclcpp::Node& n);
  void GetScan(std::vector < float > & ranges);
  void SetLaserOffset(const geometry_msgs::msg::Pose & offset);
  void SetOccupiedThreshold(const int8_t t) {assert(t >= 0 && t <= 100); occupied_threshold = t;}
  void SetLaserNoiseStdDev(const double noise) {assert(noise >= 0.0); noise_sd = noise;}
...
private:
  ...
  geometry_msgs::msg::Pose pose, offset;
